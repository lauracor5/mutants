...
import com.co.prueba.mercadoLibre.msmutantsN3.utils.Validate;
...
public class MutanServiceImpl implements MutantService {
    ...
    @Override
    public Optional<Mutant> findByDna(String dna) {
        return mutantRepository.findByDna(dna);
    }

    @Override
    @Transactional
    public Mutant save(String[] dna) {
        String completeDnaChain="";
        for (String dnaRow: dna) {
            completeDnaChain = completeDnaChain.concat(dnaRow);
        }
        Optional<Mutant>mutantOpt = findByDna(completeDnaChain);
        if(mutantOpt.isEmpty()){
            Mutant mutant = new Mutant();
            mutant.setDna(completeDnaChain);
